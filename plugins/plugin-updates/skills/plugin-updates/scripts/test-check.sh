...
out="$(run | msg)"
if printf '%s' "$out" | grep -q 'plugins: 5 current'; then ck 0 "all current: one line says how many were checked"; else ck 1 "all current: $out"; fi
...
out="$(run | msg)"
if printf '%s' "$out" | grep -q 'plugins: 5 current'; then ck 0 "inside the interval the cached lookup answers"; else ck 1 "re-fetched inside the interval: $out"; fi
...
mv "$GH" "$GH.off"
out="$(run | msg)"
if printf '%s' "$out" | grep -q 'plugins: 4 current'; then ck 0 "updating clears the report with no network"; else ck 1 "stale after update: $out"; fi
mv "$GH.off" "$GH"
